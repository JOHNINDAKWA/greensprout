...
export default function ServicesPage() {
  return <><SiteHeader/><main>
    <section className="services-hero"><Image src={photo.hydroHero} alt="Illustration of hydroseeding on prepared ground" fill priority sizes="100vw"/><div className="services-hero-shade"/><div className="page-shell services-hero-content"><p className="eyebrow light">GreenSprout services</p><h1>Practical ways to grow grass, protect soil and improve land.</h1><p>Start with the result you need. We will help you understand the right method, the work involved and the support available.</p><a className="section-scroll-cue" href="#explore-services"><span>Scroll to explore services</span><span className="section-scroll-cue-arrow" aria-hidden="true">↓</span></a></div></section>
    <section id="explore-services" className="services-list"><div className="page-shell"><div className="center-heading"><p className="eyebrow">Choose a service</p><h2>From one site visit to complete project support.</h2></div><div className="services-grid">{services.map((service)=><Link href={`/services/${service.slug}`} key={service.slug} className="services-grid-card"><Image src={service.heroImage} alt={service.name} fill sizes="(max-width:760px) 100vw, 33vw"/><div/><span>{service.name}</span><p>{service.short}</p><strong>Explore service <ArrowRight/></strong></Link>)}</div></div></section>
